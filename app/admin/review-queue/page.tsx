export default function ReviewQueuePage() {
  return (
    <div className="grid gap-6">
      <section className="rounded-[34px] border border-[var(--line)] bg-[var(--surface-strong)] p-6 sm:p-8">
        <p className="text-xs uppercase tracking-[0.35em] text-[var(--muted)]">Review queue</p>
        <h1 className="mt-4 font-display text-5xl text-[var(--foreground)]">
          Human review remains the gate before publication.
        </h1>
      </section>

      <div className="grid gap-4">
        {[
          "Verify English writing metadata suggestions",
          "Check accepted answer variants for numeric tolerances",
          "Confirm Polish oral prompts have context-ready exemplar outlines",
        ].map((item) => (
          <div
            key={item}
            className="rounded-[28px] border border-[var(--line)] bg-[var(--surface-strong)] px-5 py-5 text-sm leading-7 text-[var(--foreground)]"
          >
            {item}
          </div>
        ))}
      </div>
    </div>
  );
}
